...
int main (int argc, char **argv) { 
   ...
   struct tms t1,t2;
   int numQueries = 0;
   numPrecomputedDistances = 0;
   
   maxLeavesToVisit = 50;
   long long totalDistances = 0;
   long long totalPrecomputedDistances = 0;
   ...
   for (int i = 1 ; i < dn+1; i++){
      ...
      if (fixed){
         ...
         siz = k;
         times(&t2);
         fprintf (stderr,"kNNs found at distance %f , visited %d/%d leaves, computed %ld distances, %ld precomputed distances\n\n",r,currVisitedLeaves,maxLeavesToVisit,numDistances, numPrecomputedDistances);
         fprintf(fpt,"%d,%f,%d,%ld,%ld\n",i, r, currVisitedLeaves, numDistances, numPrecomputedDistances);
      }
      totalDistances += numDistances;
      totalPrecomputedDistances += numPrecomputedDistances;
   }
   ...
   fprintf(fpt,"Total Precomputed Distances: %ld\n", totalPrecomputedDistances);
   fprintf(fpt,"Average distance per query: %f\n", totalDistances/(float)numQueries);
   fprintf(fpt,"Average Precomputed  distance per query: %ld\n", totalPrecomputedDistances/(float)numQueries);
